...
return {
  ...
  METADATA_TIME_STAMP = function()
    local ls, m = imports()
    return ls.text_node(m.parse_date(0, os.time(), [[%Y-%m-%dT%H:%m:%S-0500]])) -- 2023-11-01T23:48:10-0500
  end,
  ...
  JOURNAL_GRAPHICS_LINK = function()
    local ls, m = imports()
    return ls.text_node(m.parse_date(0, os.time(), [[%Y/%m/Daily/%d]]))
  end,
  JOURNAL_ENTRY_STAMP = function()
    local ls, m = imports()
    return ls.text_node(m.parse_date(0, os.time(), [[%H:%M]]))
  end,
}
